...
namespace {
    ...
    std::string name = SA_OBF("Space Auth Loader");
    std::string appid = SA_OBF("S5pwAHXYOHUXJwFbZsIZQC9q");
    std::string version = SA_OBF("1.0.1");
    std::string url = SA_OBF("https://hmood.wtf");
    std::string device_label = SA_OBF("Windows Loader");

    api SpaceAuthApp(name, appid, version, url, device_label);
